...
TutorialMove::TutorialMove(TutorialManager* ActionManager)
{
...
void TutorialMove::Initialize(FbxObject3d* object, TutorialEnemy* enemy, SceneObjects* sceneObj)
{
	object_ = object;
	enemy_ = enemy;
	sceneObj_ = sceneObj;
...
void TutorialMove::Update(Input* input, GamePad* gamePad)
{
	//2点間の距離
...
void TutorialMove::Draw()
{
}

void TutorialMove::ParticleDraw(ID3D12GraphicsCommandList* cmdList)
{
	if (cmdList)
	{
...
void TutorialMove::Distance(Vector3 start, Vector3 goal)
{
	Distance_ = { goal.x - start.x, goal.y - start.y, goal.z - start.z };
}

void TutorialMove::DistanceTwoPoints(Vector3 start, Vector3 goal)
{
	Distance(start, goal);
