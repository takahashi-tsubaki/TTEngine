...
/// 静的メンバ変数の実体
/// 
Microsoft::WRL::ComPtr<ID3D12Device> Particles::device;
Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> Particles::cmdList_;
ComPtr<ID3D12RootSignature> Particles::rootsignature;
ComPtr<ID3D12PipelineState> Particles::pipelinestate;
Camera* Particles::camera_ = nullptr;
...
void Particles::StaticInitialize(ID3D12Device* Device,ID3D12GraphicsCommandList* cmdlist)
{
	// nullptrチェック
	assert(Device);
	assert(cmdlist);
...
void Particles::InitializeDescriptorHeap()
{
...
void Particles::SetDrawBlendMode(int SetblendMode)
{
...
void Particles::LoadTexture(const std::string& fileName)
{
	HRESULT result = S_FALSE;
...
//void Particles::Charge(int life,Vector3 pos,Vector3 velocity,float speed) {
//
//	speed_ = speed;
//	for ( int i = 0; i < 10; i++ )
//	{
...
//
//
//void Particles::Barrier(Vector3 pos)
//{
//	for ( int i = 0; i < 20; i++ )
//	{
//		// 追加
...
//void Particles::RandParticle(Vector3 pos)
//{
//	for ( int i = 0; i < 20; i++ )
//	{
...
void Particles::subColor(float subcolor,Particle& p) {
...
double Particles::lerp(double a,double b,double t)
{
	return a + t * ( b - a );
}
