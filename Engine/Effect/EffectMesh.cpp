...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* EffectMesh::device_ = nullptr;

void EffectMesh::StaticInitialize(ID3D12Device* device) {
	device_ = device;
...
void EffectMesh::SetName(const std::string& name) { name_ = name; }

void EffectMesh::AddVertex(const VertexPosNormalUv& vertex) { vertices.emplace_back(vertex); }

void EffectMesh::AddIndex(unsigned short index) { indices.emplace_back(index); }

void EffectMesh::SetMaterial(Material* material) { material_ = material; }

void EffectMesh::CreateBuffers() {
	HRESULT result;
...
void EffectMesh::Draw(ID3D12GraphicsCommandList* cmdList) {
	// 頂点バッファをセット
...
void EffectMesh::AddSmoothData(unsigned short indexPos, unsigned short indexVertex) {
	smoothData[indexPos].emplace_back(indexVertex);
}

void EffectMesh::CalculateSmoothedVertexNormals() {
	...
	for (; itr != smoothData.end(); itr++) {
		...
		for (unsigned short index : v) {
			normal += XMVectorSet(
			    vertices[index].normal.x, vertices[index].normal.y, vertices[index].normal.z, 0);
		}
		normal = XMVector3Normalize(normal / (float)v.size());
		// 共通法線を使用する全ての頂点データに書きこむ
		for (unsigned short index : v) {
