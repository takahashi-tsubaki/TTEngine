...
/// 静的メンバ変数の実体
/// </summary>
const std::string EffectModel::baseDirectory = "Resources/";
ID3D12Device* EffectModel::device_ = nullptr;
UINT EffectModel::descriptorHandleIncrementSize = 0;

void EffectModel::StaticInitialize(ID3D12Device* device) {
	device_ = device;

	// メッシュの静的初期化
	EffectMesh::StaticInitialize(device);
}

EffectModel* EffectModel::CreateFromOBJ(const std::string& modelname, bool smoothing) {
	// メモリ確保
	EffectModel* instance = new EffectModel;
...
void EffectModel::Initialize(const std::string& modelname, bool smoothing) {
		...
		if (key == "f") {
			...
			while (getline(line_stream, index_string, ' ')) {
				...
				if (material && material->textureFilename.size() > 0) {
					...
					// 頂点データの追加
					EffectMesh::VertexPosNormalUv vertex{};
...
void EffectModel::LoadMaterial(const std::string& directoryPath, const std::string& filename) {
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
	file.open(directoryPath + filename);
...
void EffectModel::AddMaterial(Material* material) {
	// コンテナに登録
	materials.emplace(material->name, material);
}

void EffectModel::CreateDescriptorHeap() {
...
void EffectModel::Draw(ID3D12GraphicsCommandList* cmdList) {
	// デスクリプタヒープの配列
	if (descHeap) {
